...
def generate_data_analysis_report(erd_data, lineage_data, copybook_data, job_id):
    ...
    if entities:
        ...
        for entity in entities:
            ...
            source_files = entity.get('source', {}).get('files', [])
            ...
            if attributes:
                ...
                for attr in attributes:
                    ...
                    cobol_field = attr.get('cobol_field', 'N/A')
    ...
    if lineage_data:
        ...
        if flows:
            ...
            for flow in flows:
                ...
                source_type = flow.get('source_type', 'N/A')
                dest_file = flow.get('destination_file', 'N/A')
                dest_type = flow.get('destination_type', 'N/A')
                transformations = flow.get('transformations', [])

                # Generate meaningful flow name if missing
                if not flow_name or flow_name == 'N/A':
                    operations = [t.get('operation', 'OPERATION') for t in transformations]
                    if operations:
                        flow_name = f"{source_file} → {operations[0]} → {dest_file}"
                    else:
                        flow_name = f"{source_file} Data Flow"

                lines.append(f"### Flow: {flow_name}")
                lines.append("")
                lines.append(f"**Source:** {source_file} ({source_type})")
                lines.append(f"**Destination:** {dest_file} ({dest_type})")
                lines.append("")

                if transformations:
                    lines.append("**Transformations:**")
                    for trans in transformations:
                        trans_type = trans.get('operation', 'OPERATION')
                        ...
                        if description and description != 'N/A':
                            ...
                            lines.append(f"- **{trans_type}** in {program}")
                        else:
                            lines.append(f"- **{trans_type}**")
                    lines.append("")
