...
def infer_business_meaning(field_name, cobol_field):
    """
    Generate business meaning from field name using smart pattern matching

    Args:
        field_name: Normalized field name (e.g., "apt_id")
        cobol_field: Original COBOL field name (e.g., "APT-ID")

    Returns:
        str: Inferred business meaning
    """
    import re

    name_lower = field_name.lower()

    # Pattern-based meanings (order matters - most specific first)
    patterns = [
        # Identifiers
        ...
        (r'full.*name', 'Full name'),
        (r'(^name|_name)', 'Name field'),
        ...
        (r'city', 'City name'),
        (r'state', 'State or province'),
        (r'zip|postal', 'Postal code'),
        (r'country', 'Country name'),

        # Date and time
        (r'created.*date|creation.*date', 'Creation date'),
        (r'modified.*date|updated.*date', 'Modification date'),
        (r'registration.*date', 'Registration date'),
        (r'date', 'Date field'),
        (r'time', 'Time field'),
        (r'timestamp', 'Date and time'),
        (r'year', 'Year'),
        (r'month', 'Month'),
        (r'day', 'Day'),
        (r'hour', 'Hour'),
        ...
        # Financial
        (r'amount', 'Monetary amount'),
        (r'price', 'Price value'),
        (r'cost', 'Cost value'),
        (r'total', 'Total value'),
        (r'balance', 'Balance amount'),
        (r'payment', 'Payment information'),

        # Status and flags
        (r'status', 'Status indicator'),
        (r'flag', 'Boolean flag'),
        (r'indicator', 'Indicator field'),
        (r'type', 'Type classification'),
        (r'category', 'Category'),
        ...
        # Descriptions
        (r'desc|description', 'Description text'),
        (r'notes', 'Notes or comments'),
        (r'comments', 'Comments text'),
        (r'remarks', 'Remarks text'),

        # Business domain (specific entities)
        (r'customer|cust', 'Customer information'),
        (r'appointment|appt|apt', 'Appointment information'),
        (r'service', 'Service information'),
        (r'order', 'Order information'),
        (r'product', 'Product information'),
        (r'account', 'Account information'),
        (r'transaction', 'Transaction information'),
    ]
    ...
    # Convert "apt_customer_id" → "Apt Customer Id"
    humanized = field_name.replace('_', ' ').replace('-', ' ').title()
    return f"{humanized} field"
    ...
    for ast_entity in ast_entities:
        ...
        for attr in all_attributes:
            ...
            attribute = {
                ...
                'business_meaning': infer_business_meaning(attr['name'], attr['cobol_name'])
            }
