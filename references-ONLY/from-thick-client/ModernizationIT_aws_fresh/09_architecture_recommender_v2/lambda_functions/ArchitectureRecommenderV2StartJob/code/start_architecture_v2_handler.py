...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        # Check Code Analysis V2
        code_prefix = f"{base_path}/code_analysis_v2/jobs/"
        code_job = find_latest_job(code_prefix)
        if not code_job:
            validation_errors.append("Code Analysis V2 report not found. Please run Code Analysis V2 first.")
        else:
            print(f"✓ Code Analysis V2: {code_job}")
