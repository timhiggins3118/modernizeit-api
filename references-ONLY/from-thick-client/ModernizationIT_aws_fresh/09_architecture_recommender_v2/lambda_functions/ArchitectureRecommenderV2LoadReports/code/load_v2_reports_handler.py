...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        # Load Code Analysis V2 report
        print(f"\nLoading Code Analysis V2 report...")
        code_key = f"{base_path}/code_analysis_v2/jobs/{code_job_id}/artifacts/static_analysis.json"
        try:
            response = s3_client.get_object(Bucket=BUCKET_NAME, Key=code_key)
            consolidated['code_analysis_v2'] = json.loads(response['Body'].read().decode('utf-8'))
            print(f"✓ Code Analysis V2 loaded: {len(str(consolidated['code_analysis_v2']))} bytes")
            sources_loaded += 1
        except Exception as e:
            print(f"✗ Code Analysis V2 load failed: {str(e)}")
