...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        # Generate controllers for each project
        for project in projects:
            service_name = project['service_name']
            base_package = project['base_package']
            project_base = project['base_path']

            print(f"\n=== Generating API controllers for {service_name} ===")

            # Filter API patterns for this service
            service_api_patterns = get_service_api_patterns(
                api_patterns,
            ...
            print(f"✓ Generated {len(files_created) // len(projects)} controllers for {service_name}")
...
public class {{ entity_name }}Controller {
    ...
    public ResponseEntity<{{ entity_name }}> getById(@PathVariable Long id) {
        ...
        return ResponseEntity.ok(new {{ entity_name }}());
    }
    ...
    public ResponseEntity<{{ entity_name }}> create(@Valid @RequestBody {{ entity_name }} {{ entity_name_camel }}) {
        ...
        return ResponseEntity.status(HttpStatus.CREATED).body({{ entity_name_camel }});
    }
    ...
    public ResponseEntity<{{ entity_name }}> update(
        ...
        return ResponseEntity.ok({{ entity_name_camel }});
    }
